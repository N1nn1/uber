...
public class MarrowRibFeature extends Feature<NoneFeatureConfiguration> {

    public MarrowRibFeature(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> featurePlaceContext) {
        ...
        if (!flag) {
            ...
            this.generateSimpleRib(world, blockPos, random);
            return true;
        }
    }

    private void generateSimpleRib(WorldGenLevel world, BlockPos blockPos, RandomSource random) {
        Direction windDirection = Direction.Plane.HORIZONTAL.getRandomDirection(random);
        Direction offsetDirection = windDirection.getClockWise();
        int distance = 3;
        BlockPos[] positions = new BlockPos[]{blockPos, blockPos.relative(offsetDirection, distance), blockPos.relative(offsetDirection.getOpposite(), distance)};
        for (BlockPos position : positions) {
            if (random.nextInt(3) == 0) {
                position = position.relative(Direction.Plane.HORIZONTAL.getRandomDirection(random));
            }
            this.generateMarrowColumn(world, position, UniformInt.of(2, 4).sample(random), windDirection);
        }
    }
        ...
        for (int x = -distance; x <= distance; x++) {
            for (int z = -distance; z <= distance; z++) {
                ...
                if (x * x + z * z <= distance * distance) {
                    if (world.isStateAtPosition(checkPos.below(), DripstoneUtils::isEmptyOrWaterOrLava)) {
                        flag = false;
                    }
                }
            }
